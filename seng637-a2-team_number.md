...
4. Mocks provided many challenges when testing, specifically it added complexity, fragility, a false sense of security and it limited integration validation as mentioned in the above section.  The mocks took more critical thinking than the other tests because thought had to be put into ensuring they followed the original functionality of a true object they were mocking.  This added complexity as correct behavior must had to be defined for the mock objects.  Mocks increased test fragility because changes to the real implemenation can break tests if the mock objects don't accurately represent dependencies or if an error occured the tester had to make sure it wasn't the mock causing the error which added time to the testing process.  Overall mocks probably saved time in comparision to creating real objects, but it wasn't completely cost free or error free.
